...
export function PosterDetailModal({ 
  ...
  const handleApplyBadgesAction = async (selectedBadges: string[]) => {
    ...
    try {
      ...
      if (result.success && result.poster_url) {
        // Update the poster URL in modal
        setCurrentPosterUrl(result.poster_url)
        setImageError(false)
        
        // Add the aphrodite-overlay tag
        await handleTagAction('add')
        
        // Update parent component with new poster
        onPosterUpdate(item.id, result.poster_url)
