...
async def restore_original_poster(
    ...
    try:
        ...
        # Look for cached original poster using jellyfin_id pattern
        poster_pattern = f"jellyfin_{request.jellyfin_id}_*.jpg"
        cached_posters = list(cache_dir.glob(poster_pattern))
        
        if not cached_posters:
        ...
        # Use the first matching cached poster
        original_poster_path = cached_posters[0]
        logger.info(f"Found original poster: {original_poster_path.name}")
        
        # Verify the cached poster file exists and is readable
        if not original_poster_path.exists() or not original_poster_path.is_file():
            logger.error(f"Original poster file not accessible: {original_poster_path}")
            return RestoreOriginalResponse(
                success=False,
