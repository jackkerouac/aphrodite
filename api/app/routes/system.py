...
# Import the new version manager
...
def get_version() -> str:
    """Get the current application version using the version manager."""
...
async def check_updates():
    """
    Check for available updates to the application by querying GitHub releases.
    
    Returns:
        UpdateInfoResponse: Update availability information from GitHub
    """
    try:
        success, update_info = version_manager.check_github_releases()
        
        if not success:
            # Return error information but don't raise HTTP exception
            return UpdateInfoResponse(
                success=False,
                update_available=False,
                current_version=version_manager.current_version,
                message=f"Failed to check for updates: {update_info.get('error', 'Unknown error')}"
            )
        ...
        logger.error(f"Error in check_updates endpoint: {e}")
        return UpdateInfoResponse(
            success=False,
            update_available=False,
            current_version=version_manager.current_version,
