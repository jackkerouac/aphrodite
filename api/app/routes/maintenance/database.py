"""
Database operations for maintenance.
"""
...
from fastapi import HTTPException, Response, Depends, UploadFile, File
...
from .models import BACKUP_DIR, format_file_size, DatabaseJSONEncoder, BackupCreateRequest, BackupRestoreRequest, DatabaseImportRequest

logger = logging.getLogger(__name__)

async def get_database_status(db: AsyncSession):
...
async def create_backup(request: BackupCreateRequest, db: AsyncSession):
...
async def export_database(db: AsyncSession):
...
async def restore_database(request: BackupRestoreRequest, db: AsyncSession):
...
async def import_database_settings(request: DatabaseImportRequest, db: AsyncSession):
...
async def import_database_settings_upload(file: UploadFile, confirm_restore: bool, db: AsyncSession):
