...
class ReviewDetector:
        ...
        try:
            ...
            if async_session_factory is not None:
                ...
                async with async_session_factory() as db:
                    ...
                    if config_model and config_model.value:
                        ...
                        self.logger.info("✅ [REVIEW DETECTOR] Loaded API keys using direct database access")
            ...
            if not api_keys_data:
                ...
                try:
                    async with temp_session_factory() as db:
                        ...
                        if config_model and config_model.value:
                            api_keys_data = config_model.value.get("api_keys", {})
                            self.logger.info("✅ [REVIEW DETECTOR] Loaded API keys using temporary session")
                finally:
            ...
            if api_keys_data:
                ...
                if omdb_config and len(omdb_config) > 0 and omdb_config[0].get("api_key"):
                    ...
                    self.logger.info(f"✅ [REVIEW DETECTOR] Loaded OMDb API key: {masked_key}")
                ...
                if tmdb_config and len(tmdb_config) > 0 and tmdb_config[0].get("api_key"):
                    ...
                    self.logger.info(f"✅ [REVIEW DETECTOR] Loaded TMDb Bearer token")
            else:
                self.logger.warning("⚠️ [REVIEW DETECTOR] No API keys found in settings.yaml")
            
            self._api_keys_loaded = True
            
        except Exception as e:
            self.logger.error(f"❌ [REVIEW DETECTOR] Error loading API keys: {e}")
    ...
    async def _fetch_imdb_rating(self, imdb_id: str, settings: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not self.omdb_api_key:
                self.logger.warning(f"❌ [REVIEW DETECTOR] No OMDb API key available - cannot fetch real IMDb data for {imdb_id}")
                ...
                if omdb_data:
                    ...
                    if rating is None:
                        self.logger.info(f"📊 [REVIEW DETECTOR] No IMDb rating found in OMDb data for {imdb_id}")
                        return None
                else:
                    self.logger.warning(f"📊 [REVIEW DETECTOR] OMDb API call failed for {imdb_id}")
                    return None
    ...
    async def _fetch_tmdb_rating(self, tmdb_id: str, media_type: str = "movie", settings: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not self.tmdb_api_key:
                self.logger.warning(f"❌ [REVIEW DETECTOR] No TMDb API key available - cannot fetch real TMDb data for {tmdb_id}")
                return None  # NO DEMO DATA IN PRODUCTION
            else:
                ...
                if rating_0_10 is None:
                    self.logger.info(f"📊 [REVIEW DETECTOR] TMDb API call failed for {tmdb_id}")
                    return None
    ...
    async def _fetch_rt_critics_from_omdb(self, imdb_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Check if we have an OMDb API key
            if not self.omdb_api_key:
                self.logger.warning(f"❌ [REVIEW DETECTOR] No OMDb API key available for RT Critics - cannot fetch real data")
                return None  # NO DEMO DATA IN PRODUCTION
    ...
    async def _fetch_metacritic_from_omdb(self, imdb_id: str, settings: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Check if we have an OMDb API key
            if not self.omdb_api_key:
                self.logger.warning(f"❌ [REVIEW DETECTOR] No OMDb API key available for Metacritic - cannot fetch real data")
                return None  # NO DEMO DATA IN PRODUCTION
