...
class ResolutionBadgeApplicator:
    ...
    async def apply_resolution_badge(
        ...
        try:
            self.logger.debug(f"🛠️ apply_resolution_badge called with resolution: {resolution_data}")
            ...
            self.logger.debug(f"🚀 [DEBUG] ISOLATION: Running v1 resolution badge creation in separate thread")
            ...
            def run_v1_badge_creation():
                ...
                try:
                    ...
                    print(f"Creating resolution badge with settings - Badge Size: {settings.get('General', {}).get('general_badge_size')}, Image Badges: {settings.get('ImageBadges', {}).get('enable_image_badges')}, Use Dynamic: {settings.get('General', {}).get('use_dynamic_sizing')}")
                    ...
                    if not resolution_badge:
                        print("Failed to create resolution badge")
                        return None
                    
                    print(f"Created badge dimensions: {resolution_badge.size} (Dynamic: {general_settings.get('use_dynamic_sizing')})")
                    ...
                    print(f"Successfully applied resolution badge: {final_output_path}")
                    return final_output_path
                    
                except Exception as e:
                    print(f"V1 resolution badge creation error in thread: {e}")
                    return None
            
            # Run the entire v1 badge creation process in a thread pool
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(run_v1_badge_creation)
                result_path = await asyncio.wrap_future(future)
            
            self.logger.debug(f"🚀 [DEBUG] ISOLATION: Resolution badge creation thread completed, result: {result_path}")
