// Test revised scaling calculations for different poster sizes
...
console.log("Revised scaling calculations for different poster sizes:\n");
...
testPosters.forEach(poster => {
  ...
  // Calculate margin scale factor using the new approach
  let marginScaleFactor;
  if (scaleFactor >= 1) {
    // For larger posters, scale margins up more conservatively
    marginScaleFactor = 1 + (scaleFactor - 1) * 0.5; // Only scale up by 50% of the size increase
  } else {
    // For smaller posters, scale margins down more conservatively
    marginScaleFactor = Math.max(scaleFactor * 1.2, 0.8); // Scale down less aggressively, minimum 80%
  }
