...
export async function renderResolutionBadge(canvas, settings, metadata, sourceImagePath, fonts) {
  ...
  try {
    ...
    console.error(`Error rendering resolution badge: ${error.message}`);
    // Create a simple fallback badge
    return createFallbackBadge(canvas, 'Resolution Error');
  }
}
...
async function renderImageBadge(canvas, settings, imagePath) {
  try {
    ...
    try {
      return await renderTextBadge(canvas, settings, settings.value || 'HD');
    } catch (textError) {
      console.error('Fallback text rendering also failed:', textError);
      return createFallbackBadge(canvas, 'Resolution');
    }
  }
}
...
async function renderTextBadge(canvas, settings, text, fonts) {
  try {
    ...
    return createFallbackBadge(canvas, text || 'Resolution');
  }
}
